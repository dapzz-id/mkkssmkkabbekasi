...
<div class="p-4 sm:p-6 lg:p-8 space-y-6">
            ...
            <div class="flex items-center gap-3.5">
                ...
                <div>
                    ...
                    <p class="text-xs sm:text-sm text-slate-500 mt-0.5 font-medium">
                        Tambah agenda kegiatan MKKS SMK Kab Bekasi
                    </p>
                </div>
            </div>
    ...
    <div class="bg-white rounded-2xl border border-slate-200/80 shadow-xs overflow-hidden">
        <div class="px-6 py-4 border-b border-slate-100 bg-slate-50/50 flex items-center justify-between">
            <h2 class="text-base font-bold text-slate-900">Form Tambah Agenda</h2>
            <span class="text-xs text-slate-500 font-medium">* Wajib diisi</span>
        </div>

        <form action="{{ route('calendar.store') }}" id="formTambahCalendar" method="POST" class="p-6 sm:p-8 space-y-6">
            @csrf
            ...
            <div>
                ...
                <input type="text"
                       ...
                       value="{{ old('name') }}"
                       required
                       placeholder="Contoh: LKS SMK Tingkat Kabupaten Bekasi 2026"
...
document.addEventListener('DOMContentLoaded', function() {
    const btnSubmit = document.getElementById('btnSubmitCalendar');
    const form = document.getElementById('formTambahCalendar');
    ...
    if (btnSubmit && form) {
        btnSubmit.addEventListener('click', function() {
            ...
            Swal.fire({
                title: "Simpan Agenda Kegiatan?",
                text: "Pastikan nama acara dan tanggal sudah sesuai.",
                icon: "question",
