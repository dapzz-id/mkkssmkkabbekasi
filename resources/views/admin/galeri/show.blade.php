...
<div class="p-4 sm:p-6 lg:p-8 space-y-6">
    ...
    <div class="flex flex-col gap-4 pt-1 sm:pt-2">
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div class="flex items-center gap-3.5">
                ...
                <div>
                    ...
                    <p class="text-xs sm:text-sm text-slate-500 mt-0.5 font-medium">
                        Detail data galeri kegiatan
                    </p>
                </div>
            </div>

            <!-- Action Buttons: Edit (if permitted) & Kembali -->
            <div class="flex items-center gap-2.5 shrink-0">
                ...
                @if(Auth::user()->role === 'superadmin' || Auth::user()->divisi_uuid == $galeri->divisi_uuid)
                    <a href="{{ url('/gallery/' . $galeri->uuid . '/edit') }}"
        ...
        <div class="pt-1.5">
            <x-admin-breadcrumb :items="[
                ...
                ['label' => \Illuminate\Support\Str::limit($galeri->judul, 35)]
            ]" />
        </div>
    </div>
    ...
    <div class="bg-white rounded-2xl border border-slate-200/80 shadow-xs overflow-hidden divide-y divide-slate-100">
        ...
        <div class="p-6 sm:p-8 space-y-4">
            <div class="flex flex-wrap items-center gap-2">
                <span class="px-3 py-1 rounded-full text-xs font-bold bg-blue-50 text-blue-700 border border-blue-200">
                    Divisi: {{ $galeri->divisi->nama_divisi ?? 'Umum' }}
                </span>
            </div>
            ...
            <div class="flex flex-wrap items-center gap-4 text-xs sm:text-sm text-slate-500 pt-1 border-t border-slate-100">
                <div class="inline-flex items-center gap-1.5">
                    ...
                    <span>{{ \Carbon\Carbon::parse($galeri->tanggal_upload)->translatedFormat('l, d F Y - H:i') }} WIB</span>
                </div>
                <div class="inline-flex items-center gap-1.5">
                    <svg class="w-4 h-4 text-slate-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" />
                    </svg>
                    <span>Pengunggah: <strong>{{ $galeri->user->name ?? 'Admin' }}</strong></span>
                </div>
            </div>
        ...
        @php
            $mediaList = json_decode($galeri->url_media, true) ?? [];
        @endphp
...
<!-- MEDIA LIGHTBOX / LARGE PREVIEW MODAL -->
<div id="lightboxModal"
     class="fixed inset-0 z-50 bg-slate-950/95 backdrop-blur-md hidden flex flex-col items-center justify-between p-3 sm:p-6 select-none"
     role="dialog"
     aria-modal="true"
     aria-label="Media Preview">

    <!-- Persistent Floating Close Button (Always visible on mobile & desktop) -->
    <button type="button"
            id="btnCloseLightbox"
    ...
    <!-- Top Bar: Counter & Title (With right padding so it never overlaps with close button) -->
    <div class="w-full max-w-6xl flex items-center gap-2.5 text-white z-10 pt-1 sm:pt-0 pr-14 sm:pr-16 min-w-0">
        ...
        <span id="lightboxTitle" class="truncate text-xs sm:text-sm text-white/90 font-semibold min-w-0 flex-1">{{ $galeri->judul }}</span>
    </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Click outside media (anywhere on the dark overlay or empty area) to close
    modal.addEventListener('click', function(e) {
        // Prevent close only if clicking on image, video, or navigation buttons
    ...
    // Touch Swipe Gestures for mobile (swipe left/right to navigate, swipe down to close)
    let touchStartX = 0;
    let touchStartY = 0;
    let touchEndX = 0;
    let touchEndY = 0;
    ...
    function handleSwipe() {
        ...
        // Vertical swipe down (threshold 70px) to close
