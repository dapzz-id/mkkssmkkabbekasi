            ...
            <div class="flex items-center gap-3.5">
                ...
                <div>
                    ...
                    <p class="text-xs sm:text-sm text-slate-500 mt-0.5 font-medium">
                        Upload foto pimpinan dengan crop frame tetap 4:3
                    </p>
                </div>
            </div>
    ...
    <div class="bg-white rounded-2xl border border-slate-200/80 shadow-xs overflow-hidden">
        <div class="p-6 sm:p-8">
            <div class="mb-6 pb-4 border-b border-gray-100">
                <h2 class="text-xl font-bold text-gray-800">Form Tambah Pimpinan</h2>
                <p class="text-sm text-gray-500 mt-1">
                    Upload foto pimpinan, sesuaikan posisi wajah pada fixed frame 4:3, dan lengkapi identitas.
                </p>
            </div>
            ...
            <form id="formTambahPimpinan" enctype="multipart/form-data" novalidate>
                @csrf

                <!-- Section: UPLOAD & CROP FOTO -->
                <div class="mb-7 bg-slate-50/70 p-4 sm:p-5 rounded-xl border border-slate-200/80">
                    <label class="block text-sm font-bold text-gray-800 mb-1">
                        Foto Pimpinan <span class="text-red-500">*</span>
                    </label>
                    <p class="text-xs text-gray-500 mb-3">
                        Frame foto menggunakan rasio tetap <strong>4:3</strong> (sesuai frame landing page). Anda dapat menggeser posisi foto dan melakukan zoom agar wajah terlihat jelas.
                    </p>

                    <!-- Dropzone Container -->
                    <div id="dropzone"
                         class="relative border-2 border-dashed border-gray-300 hover:border-blue-400 bg-white rounded-xl p-6 text-center transition-all cursor-pointer focus:outline-none focus:ring-2 focus:ring-blue-400"
                         tabindex="0"
                         role="button"
                         aria-label="Upload foto pimpinan. Tarik dan lepas atau klik untuk memilih file">
                        ...
                        <div id="dropzoneDefaultState" class="flex flex-col items-center">
                            <div class="w-14 h-14 mb-3 rounded-full bg-blue-50 text-blue-600 flex items-center justify-center">
                                <svg xmlns="http://www.w3.org/2000/svg" class="w-7 h-7" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z" />
                                </svg>
                            </div>
                            <p class="text-sm font-semibold text-gray-700">
                                <span class="text-blue-600 hover:underline">Tarik & Lepas Foto di sini</span> atau Klik untuk Memilih
                            </p>
                            <p class="text-xs text-gray-400 mt-1">Mendukung format: JPG, JPEG, PNG, WEBP (Maks. 2MB)</p>
                        </div>

                        <div id="dropzoneDragState" class="hidden flex flex-col items-center py-2">
                            <div class="w-14 h-14 mb-2 rounded-full bg-blue-100 text-blue-700 flex items-center justify-center animate-bounce">
                                <svg xmlns="http://www.w3.org/2000/svg" class="w-7 h-7" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 16a4 4 0 01-.88-7.903A5 5 0 1115.9 6L16 6a5 5 0 011 9.9M15 13l-3-3m0 0l-3 3m3-3v12" />
                                </svg>
                            </div>
                            <p class="text-sm font-bold text-blue-700">Lepaskan file foto untuk memulai crop</p>
                        </div>
                    </div>

                    <!-- Client Error Alert for Upload -->
                    ...
                    <!-- Real Upload Progress Bar (Only shown during form submission upload) -->
                    <div id="uploadProgressSection" class="hidden mt-4 p-3.5 bg-white border border-blue-100 rounded-xl shadow-sm">
                        <div class="flex justify-between items-center text-xs font-semibold text-gray-700 mb-1.5">
                        ...
                        <div class="w-full bg-gray-200 rounded-full h-2.5 overflow-hidden">
                            <div id="progressBarFill" class="bg-blue-600 h-2.5 rounded-full transition-all duration-150 ease-out" style="width: 0%"></div>
                        </div>
                        <p id="progressByteDetails" class="text-[11px] text-gray-400 mt-1 text-right">0 KB / 0 KB</p>
                    </div>

                    <!-- Crop Editor Modal / Container -->
                    <div id="cropEditorContainer" class="hidden mt-5 pt-4 border-t border-gray-200">
                        <div class="flex items-center justify-between mb-3">
                            <h3 class="text-sm font-bold text-gray-800 flex items-center gap-1.5">
                                <span class="w-2 h-2 rounded-full bg-blue-600"></span>
                                Sesuaikan Foto (Frame Tetap 4:3)
                            </h3>
                            <button type="button" id="btnResetCrop" class="text-xs text-blue-600 hover:text-blue-800 font-medium px-2 py-1 rounded bg-blue-50 hover:bg-blue-100 transition-colors">
                                Reset Posisi
                            </button>
                        </div>

                        <!-- Editor & Live Landing Page Preview Grid -->
                        <div class="grid grid-cols-1 md:grid-cols-12 gap-5 items-start">
                            <!-- Left: Cropper Canvas -->
                            <div class="md:col-span-7 bg-gray-900 rounded-xl overflow-hidden p-1 shadow-inner">
                                <div class="max-h-[360px] flex items-center justify-center overflow-hidden">
                                    <img id="cropperImage" src="" alt="Foto untuk di-crop" class="max-w-full block">
                                </div>
                                
                                <!-- Cropper Controls -->
                                <div class="p-3 bg-gray-800/90 text-white rounded-b-lg flex flex-col gap-2.5">
                                    <div class="flex items-center justify-between text-xs text-gray-300">
                                        <span>Zoom:</span>
                                        <div class="flex items-center gap-2">
                                            <button type="button" id="btnZoomOut" title="Zoom Out" class="w-7 h-7 flex items-center justify-center bg-gray-700 hover:bg-gray-600 rounded text-base font-bold transition-colors">-</button>
                                            <input type="range" id="zoomSlider" min="0.2" max="3" step="0.02" value="1" class="w-28 sm:w-36 accent-blue-500 cursor-pointer">
                                            <button type="button" id="btnZoomIn" title="Zoom In" class="w-7 h-7 flex items-center justify-center bg-gray-700 hover:bg-gray-600 rounded text-base font-bold transition-colors">+</button>
                                        </div>
                                    </div>
                                    <p class="text-[11px] text-gray-400 text-center">
                                        💡 Geser foto untuk menempatkan wajah di tengah frame. Frame 4:3 telah terkunci sesuai landing page.
                                    </p>
                                </div>
                            </div>

                            <!-- Right: Live Landing Page Preview Card -->
                            <div class="md:col-span-5 flex flex-col items-center bg-white p-4 rounded-xl border border-gray-200 shadow-sm">
                                <div class="w-full flex items-center justify-between mb-2">
                                    <span class="text-xs font-bold text-gray-700">Preview Landing Page</span>
                                    <span class="text-[10px] bg-green-100 text-green-700 font-semibold px-2 py-0.5 rounded-full">Rasio 4:3 Tetap</span>
                                </div>
                                ...
                                <div class="preview-landing-card">
                                    ...
                                    <div class="p-3 text-center">
                                        <p id="previewCardName" class="text-xs font-bold text-gray-800 truncate">Nama Pimpinan</p>
                                        <p id="previewCardPosition" class="text-[11px] text-gray-500 truncate mt-0.5">Jabatan Pimpinan</p>
                                    </div>
                                </div>

                                <p class="text-[11px] text-gray-400 mt-2 text-center">
                                    Inilah tampilan persis foto pada kartu pimpinan di halaman utama.
                                </p>

                                <button type="button" id="btnApplyCrop" class="w-full mt-3 px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white text-xs font-semibold rounded-lg shadow-sm transition-colors flex items-center justify-center gap-1.5">
                    ...
                    <div id="cropSuccessNotice" class="hidden mt-3 p-3 bg-emerald-50 border border-emerald-200 rounded-xl flex items-center justify-between">
                        <div class="flex items-center gap-3">
                            <div class="w-12 h-9 rounded overflow-hidden border border-emerald-300 shadow-sm">
                                <img id="croppedThumbnail" src="" class="w-full h-full object-cover" alt="Thumbnail Crop">
                            </div>
                            <div>
                                <p class="text-xs font-bold text-emerald-800 flex items-center gap-1">
                                    <svg class="w-3.5 h-3.5" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clip-rule="evenodd"/></svg>
                                    Foto Berhasil Di-crop (4:3)
                                </p>
                                <p id="cropOutputMeta" class="text-[11px] text-emerald-600">Resolusi optimal 800 × 600 px</p>
                            </div>
                        </div>
                ...
                <div class="flex flex-col sm:flex-row gap-3 pt-4 border-t border-gray-100">
                    <button type="submit"
                        ...
                        <span id="btnSubmitText">Simpan Pimpinan</span>
                    </button>
                    <a href="/pimpinan"
...
@push('scripts')
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            // Elements
            const form = document.getElementById('formTambahPimpinan');
            ...
            let cropper = null;
            let croppedBlob = null; // Holds the processed 4:3 cropped image
            ...
            // Accessibility: trigger file input with Enter/Space on dropzone
            ...
            function initCropper(imageSrc) {
                ...
                // Wait for image render before starting Cropper
                cropperImage.onload = function() {
                    cropper = new Cropper(cropperImage, {
                        aspectRatio: 4 / 3, // STRICTLY FIXED 4:3
                        viewMode: 1, // Restrict crop box within image canvas
                        dragMode: 'move', // User moves the image
                        autoCropArea: 0.9,
                        responsive: true,
                        restore: false,
                        guides: true,
                        center: true,
                        highlight: false,
                        cropBoxMovable: false, // Disables moving the frame
                        cropBoxResizable: false, // Disables resizing frame
                ...
                // Scroll gently to cropper if needed
            ...
            function applyCropResult(callback) {
                ...
                // Normalized 4:3 high quality output (800 x 600 px)
            ...
            // Form Submission with REAL Upload Progress & Button Loading State
            form.addEventListener('submit', function(e) {
                ...
                // Ensure photo is cropped and ready
                if (!croppedBlob) {
                    if (cropper) {
                        ...
                        showUploadError('Silakan pilih dan sesuaikan foto pimpinan terlebih dahulu.');
            ...
            function executeSubmission() {
                ...
                formData.append('foto', croppedBlob, 'pimpinan_' + Date.now() + '.jpg');

                const xhr = new XMLHttpRequest();
                xhr.open('POST', '{{ route("pimpinan.store") }}', true);
                ...
                xhr.onload = function() {
                    if (xhr.status >= 200 && xhr.status < 300) {
                        ...
                        Swal.fire({
                            icon: 'success',
                            title: 'Berhasil!',
                            text: res.message || 'Data pimpinan berhasil disimpan.',
                            timer: 1500,
                        ...
                        btnSubmitText.textContent = 'Simpan Pimpinan';
                        ...
                        if (errorData.errors) {
                            ...
                            Swal.fire('Gagal Menyimpan', errorData.message || 'Terjadi kesalahan sistem saat menyimpan foto pimpinan.', 'error');
                        }
                    }
                };
