...
@if(!empty($siteKey))
    <div class="turnstile-container my-3 flex flex-col items-center justify-center">
        ...
        @error('cf-turnstile-response')
            <div class="text-red-300 text-xs mt-1 text-center font-semibold" role="alert">
                {{ $message }}
            </div>
        @enderror
    </div>
                        ...
                        if (widget) {
                            ...
                            if (!tokenInput || !tokenInput.value) {
                                ...
                                if (!alertEl) {
                                    ...
                                    alertEl.className = 'text-amber-300 text-xs mt-1 text-center font-semibold animate-pulse';
                                    widget.parentNode.appendChild(alertEl);
                                }
                                alertEl.textContent = 'Sedang memverifikasi keamanan... Silakan tunggu sejenak sebelum login.';
                            }
                        }
                    });
                });
